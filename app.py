...
def throughput_upload():
    ...
    return jsonify({"status": "ok", "size_bytes": len(received)})

@app.route("/ir-test", methods=["GET"])
def ir_test():
    """
    Pengujian Information Rate (IR) — ITU-T Y.1564
    Contoh: /ir-test?cir_mbps=10&duration=30
    """
    try:
        cir = float(request.args.get("cir_mbps", 10))
        duration = float(request.args.get("duration", 30))
    except:
        return jsonify({"error": "Parameter harus numerik"}), 400

    if cir <= 0 or duration <= 0:
        return jsonify({"error": "cir_mbps dan duration harus > 0"}), 400

    payload = b"x" * 1024  # 1 KB
    bits_per_second = cir * 1024 * 1024
    packets_per_second = bits_per_second / (len(payload) * 8)
    ...
    def generate():
        ...
        ir_tx = (sent * len(payload) * 8) / (1024 * 1024 * duration)
        print(f"[IR TEST] CIR={cir} Mbps | IR_Tx={ir_tx:.2f} Mbps | Duration={duration}s")
...
def throughput_download():
    ...
    if "size_mb" not in request.args:
        return jsonify({"error": "Parameter 'size_mb' wajib ada, contoh: /throughput_download?size_mb=10"}), 400

    # Validasi apakah integer
    try:
        size_mb = int(request.args["size_mb"])
    except ValueError:
        return jsonify({"error": "size_mb harus berupa angka"}), 400

    # Validasi rentang 5–50 MB
    if size_mb < 5 or size_mb > 50:
        return jsonify({"error": "size_mb harus di antara 5 dan 50 MB"}), 400
